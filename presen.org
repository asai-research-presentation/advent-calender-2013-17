...
*たまたま* Guy Steele がその論文を読んで(弟子だった?)、
しかも機械語で実装してしまいました(雑)。
...
どうやらNASA JPL (Jet Propulsion Laboratory) はそれ関連でずっとLispを
使っているらしく、
プログラムのアップロードにはAllegro CLのREPLで関数を再定義する感じで
行なっているそうです。

それにたしか、衛星などのメモリはそこまで潤沢ではなく(宇宙線対策などで安全性重視)、
プログラム全部積みで打ち上げるのは無茶だ、みたいな話を聞きました。
また、打ち上げ期日は惑星の位置などに厳密に決められてしまうので、
開発が間に合わなかったらプログラムはあとで送るしか無いわけです。
そもそも追加ミッションなども必要ですから。

こういった機能は、Common Lispぐらい動的なシステムでしか実現できません。

まあ、いろいろと伝聞口調で申し訳ありません。よく知らないので。
福永先生からFollow up がはいるかも。

** 天体望遠鏡にもプランナが乗っている

ハッブル望遠鏡には [[http://www.stsci.edu/institute/software_hardware/spike/][SPIKE]] とよばれるプランナが乗っています。
望遠鏡には、無数の天文学者から、
バラバラに複数の観測目標のリクエストが送られてきます。
しかし、宇宙空間に居る望遠鏡は、姿勢を変えるのに非常に時間がかかります。
そこで、プランニングを用いて望遠鏡のアクション数を最適化することが重要になります。

なおこの内容は +鈍器+ 例の教科書に乗っています(^^;)
